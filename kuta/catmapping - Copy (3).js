...
var ChildCollection = Backbone.Collection.extend({
	...
	addChild: function(parent_id, el){
		var that = this;
		var coll = new ChildCollection({
			parent_id: parent_id,
		...
		coll.fetch({success: function(c, r){ 
			
			if(r.length > 0) { 
				that.children.push(coll);
				var node = $(el).tree('getNodeById', c.parent_id);
				$(el).tree('loadData', r, node); 
			}
		}}); 
	}
...
var TreeView = Backbone.View.extend({
	...
	treeSelected: function(event) {
		...
		if (event.node && event.node.children.length === 0){
			...
			this.collection.addChild(pid, this.el); 
		};
		...
		//this.treeUpdate(this);
	},

	treeUpdate: function(context) {
		...
		that.collection.fetch({
			success: function(c, r) {
				...
				if (that.collection.children.length === 0) {
					//there are no children
					return;
				} else {
					//there are children

					for (var i = that.collection.children.length - 1; i >= 0; i--) {
						console.log("********************" + i);
						var childcoll = that.collection.children[i];
						console.log(that.collection.children[i].parent_id);

						childcoll.fetch({success: function(cc, rr) {
								var data = rr;
								var node = $(that.el).tree('getNodeById', cc.parent_id);
								console.log("node");
								console.log(node)
								$(that.el).tree('loadData', data, node);
								console.log("data loaded into tree" + rr);
							}
						})
						//var data = that.collection.children[i].toJSON;
						//var node = $(that.el).tree('getNodeById', that.collection.children[i].parent_id);
						//$(that.el).tree('loadData', data, node);
					};
				}
			}
